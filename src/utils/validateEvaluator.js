...
const evaluatorSchema = (componentList, pythonSrc) =>
  object({
    ...
    className: string("Class Name must be a string").when("type", {
      ...
      otherwise: () =>
        string("Class Name must be a string") // type is TargetValueEvaluator or default
    ...
    keyRequests: array("Key Requests must be an array").of(
      object({
        ...
        subsystem: string("Subsystem must be a string")
          ...
          .test(
            "no-injection",
            "Subsystem contains invalid characters",
            noInjection,
          )
          .test(
            "subsystem-exists",
            "Subsystem does not exist",
            (value, { parent: keyRequest }) => {
              const { asset } = keyRequest;
              return (
                componentList.find(
                  (component) =>
                    component.id === value && component.parent === asset,
                ) !== undefined
              );
            },
          ),
        ...
        if (name === "keyRequests") {
          value.forEach((keyRequest, index) => {
            try {
              ...
              evaluatorSchema(componentList, pythonSrc).validateSyncAt(
                `keyRequests[${index}]`,
                evaluator,
              );
