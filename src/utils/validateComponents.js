...
function parameterValueError(value, type, key, createError) {
  ...
  switch (type) {
    ...
    case "double":
      error =
        Number(value) !== parseFloat(value) ? `${key} must be a number` : null;
      break;
    case "int":
      error =
        Number(value) !== parseInt(value) ? `${key} must be an integer` : null;
      break;
    case "bool":
...
const assetSchema = (componentList) =>
  object({
    name: string("Asset Name must be a string")
      ...
      .test("unique-name", "Asset Name must be unique", function (value) {
        return (
          componentList.filter((component) => component.name === value)
            .length === 1
        );
      }),
    ...
    stateData: array("State Data must be an array")
      .of(
        number("State Data components must be numbers").typeError(
          "State Data components must be numbers",
        ),
      )
      .required("State Data is required")
      .test(
        "state-data",
        ({ value }) => vectorError(value, "State Data"),
        (value) => vectorError(value, "State Data") === null,
      )
      .test(
        "state-data-position-unique",
        "State Data Position must be unique",
        function ([x, y, z]) {
          return (
            componentList.filter(({ stateData }) => {
              if (!stateData) return false;
              return (
                stateData[0] == x && stateData[1] == y && stateData[2] == z
              );
            }).length === 1
          );
        },
    ...
    integratorOptions: object({
      h: number("h must be a number")
        .required("h is required")
        .typeError("h must be a number"),
      rtol: number("rtol must be a number")
        .required("rtol is required")
        .typeError("rtol must be a number"),
      atol: number("atol must be a number")
        .required("atol is required")
        .typeError("atol must be a number"),
      eps: number("eps must be a number")
        .required("eps is required")
        .typeError("eps must be a number"),
      nSteps: number("nSteps must be a number")
        .required("nSteps is required")
        .typeError("nSteps must be a number"),
...
const subsystemSchema = (componentList, pythonSrc) =>
  object({
    name: string("Subsystem Name must be a string")
      ...
      .test(
        "no-injection",
        "Subsystem Name contains invalid characters",
        noInjection,
      ),
    parent: string("Parent must be a string")
      ...
      .test("parent-exists", "Parent does not exist", (value) => {
        return (
          componentList.find((component) => component.id === value) !==
          undefined
        );
      }),
    type: string("Subsystem Type must be a string")
      ...
      .test(
        "no-injection",
        "Subsystem Type contains invalid characters",
        noInjection,
      ),
    src: string("Source must be a string").when("type", {
      is: "scripted",
      then: () =>
        string("Source must be a string")
          ...
          .test(
            "valid-python-source",
            "Source must be in the Python source directory listed in the scenario parameters.",
            function (value) {
    ...
    className: string("Class Name must be a string").when("type", {
      is: "unscripted",
      then: () =>
        string("Class Name must be a string")
          .min(1, "Class Name must be at least 1 character")
          .required("Class Name is required")
          .matches(
            /^[a-zA-Z_][a-zA-Z0-9_]*$/,
            "Class name must start with a letter or underscore and contain only letters, numbers, and underscores",
          )
          .test(
            "no-injection",
    ...
    parameters: array("Parameters must be an array").of(
      object({
    ...
    states: array("States must be an array").of(
      object({
        name: string("Name must be a string")
          .min(1, "Name must be at least 1 character")
          .required("Name is required")
    ...
    try {
      ...
      switch (name) {
        case "integratorOptions":
          Object.entries(component.integratorOptions).forEach(
            ([key, value]) => {
              try {
                assetSchema(componentList).validateSyncAt(
                  `integratorOptions.${key}`,
                  component,
                );
        ...
        case "parameters":
          component.parameters.forEach((parameter, index) => {
            try {
              subsystemSchema(componentList, pythonSrc).validateSyncAt(
                `parameters[${index}]`,
                component,
              );
              // Remove error message from the name key of the object
