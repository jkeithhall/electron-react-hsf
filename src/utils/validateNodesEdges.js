...
import {
  ...
  subcomponentType,
  modelEdgeConfig,
} from "./createModelNodesEdges";

function isValidRGBColor(string) {
  return /rgba?\((\d{1,3}), (\d{1,3}), (\d{1,3})(, \d?(\.\d{1,2})?)?\)/.test(
    string,
  );
}

const assetNodeSchema = (componentList) =>
  object({
    id: string("ID must be a string")
      .required("ID is required")
      .test("no-injection", "ID contains invalid characters", noInjection),
    type: string("Type must be a string").test(
      "asset-type",
      "Type must be an asset",
    ...
    data: object({
      backgroundColor: string("Background Color must be a string")
        ...
        .test(
          "valid-rgb-color",
          "Background Color must be a valid RGB color",
          isValidRGBColor,
        ),
      ...
      data: object().test(
        "deep-equal",
        "Data must equal component in componentList",
        function (value) {
          const { id } = value;
          const component = componentList.find(
            (component) =>
              component.id === id && component.parent === undefined,
          );
          return (
            component && JSON.stringify(value) === JSON.stringify(component)
          );
        },
      ),
    }),
...
const subsystemNodeSchema = (modelNodes, componentList) =>
  object({
    id: string("ID must be a string")
      .required("ID is required")
      .test("no-injection", "ID contains invalid characters", noInjection),
    type: string("Type must be a string").test(
      "subcomponent-type",
      "Type must be a subcomponent",
      (value) => value === subcomponentType,
    ),
    position: object({
      x: number("X must be a number")
        .required("X is required")
    ...
    parentNode: string("Parent Node must be a string")
      ...
      .test(
        ...
        function (value) {
          return (
            modelNodes.find(
              (node) => node.id === value && node.type === "asset",
            ) !== undefined
          );
        },
      ),
...
const edgeSchema = (modelNodes) =>
  object({
    id: string("ID must be a string")
      .required("ID is required")
      .test("no-injection", "ID contains invalid characters", noInjection),
    data: string("Data must be a string").test(
      "no-injection",
    ...
    markerEnd: object().test(
      "valid-marker-end",
      "Marker End must be a valid marker end",
      function (value) {
        return (
          JSON.stringify(value) === JSON.stringify(modelEdgeConfig.markerEnd)
        );
      },
    ),
