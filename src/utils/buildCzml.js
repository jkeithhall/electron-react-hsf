...
function addAssetPacketsToCzml(
  czml,
  assets,
  startDatetime,
  endDatetime,
...
function addTaskPacketsToCzml(czml, taskList) {
  ...
  Object.values(targets).forEach((target) =>
    czml.push(targetToCzmlPacket(target)),
  );
}
...
function getAccessShowIntervals(
  taskIntervals,
  startSeconds,
  endSeconds,
  startDate,
  ...
  for (let seconds = startSeconds; seconds < endSeconds; seconds++) {
    const isInTaskInterval = taskIntervals.some(
  ...
  for (let i = 1; i < oneSecondIntervals.length; i++) {
    ...
    if (interval.boolean === currentInterval.boolean) {
      ...
      const startISO = startDate
        .clone()
        .add(currentInterval.start, "seconds")
        .format();
      const endISO = startDate
        .clone()
        .add(currentInterval.end, "seconds")
        .format();
      showIntervals.push({
...
async function addAccessesToCzml(
  czml,
  outputPath,
  fileName,
  startDate,
  ...
  Object.entries(accessIntervals).forEach(
    ([accessName, { taskIntervals, eventIntervals }]) => {
      const showIntervals = getAccessShowIntervals(
        taskIntervals,
        startSeconds,
        endSeconds,
        startDate,
...
async function buildCzmlPackets(
  name,
  version,
  startJD,
  startSeconds,
