...
export default function AddComponentDial({
  componentList,
  onLayout,
  handleNewNodeClick,
}) {
  const assetCount = componentList.filter(
    (component) => component.parent === undefined,
  ).length;

  const handleAutoLayoutClick = (e) => {
    if (e.shiftKey) {
      // If shift key is pressed, layout horizontally
      if (e.altKey) {
        // If option/alt key is pressed, layout left to right
        onLayout("LR");
      } else {
      ...
      if (e.altKey) {
        // If option/alt key is pressed, layout bottom to top
        onLayout("BT");
      } else {
    ...
    >
      ...
      {assetCount > 0 && (
        <SpeedDialAction
          ...
          enterDelay={500}
          onClick={() => {
            handleNewNodeClick("newComponentEditor");
          }}
        />
      )}
      <SpeedDialAction
        ...
        enterDelay={500}
        onClick={() => {
          handleNewNodeClick("newAssetEditor");
        }}
      />
    </SpeedDial>
  );
