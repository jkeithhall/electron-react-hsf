...
const constraintTypeOptions = [
  ...
  "FAIL_IF_EQUAL",
  "FAIL_IF_NOT_EQUAL",
  "FAIL_IF_HIGHER_OR_EQUAL",
  "FAIL_IF_LOWER_OR_EQUAL",
];

const DeleteConstraintButton = ({
  markedForDeletion,
  index,
  hovered,
  buttonRef,
  ...
  if (markedForDeletion !== index) {
    ...
    return (
      <Button
        ...
        onClick={(e) => handleDeleteClicked(e, index)}
        startIcon={
          <RemoveCircleIcon sx={{ "&.MuiSvgIcon-root": { fontSize: 22 } }} />
        }
        fontSize="large"
      >
        Confirm
      ...
      <>
        ...
        {constraints.map((constraint, index) => {
          ...
          return (
            <Card
              key={constraint.id}
              ref={(element) => (ref[stateKey] = element)}
              sx={{ marginTop: 1, padding: 1, borderRadius: 2 }}
            >
              <Stack
                direction="row"
                alignItems="center"
              ...
              >
                ...
                >
                  {convertDisplayName(constraint.stateKey)}
                </Typography>
              ...
              <Grid container spacing={2} mt={1}>
                <Grid item xs={6}>
                  <TextField
                    ...
                    onBlur={(e) => handleBlur(e, constraint.id)}
                  >
                    {constraintTypeOptions.map((type) => (
