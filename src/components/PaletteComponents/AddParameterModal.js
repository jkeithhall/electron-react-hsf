...
export default function AddParameterModal({
  ...
  const handleChangeType = (e) => {
    ...
    if (value === "vector" && prevType !== "vector") {
      ...
      (value === "int" && prevType !== "int") ||
      (value === "double" && prevType !== "double")
    ) {
      setParameterVector([0, 0, 0]);
      setVectorLength(3);
      setParameterValue(0);
  ...
  const handleBlur = () => {
    if (parameterType === "int") {
      ...
      if (nonNumberComponents.length > 1) {
        setValueErrorMessage(
          `Vector components ${nonNumberComponents.join(", ")} are not numbers`,
        );
      } else if (nonNumberComponents.length === 1) {
        setValueErrorMessage(
          `Vector component ${nonNumberComponents[0]} is not a number`,
        );
      }
    }
  };
    ...
    <div className="overlay">
      ...
      >
        ...
        {label !== "Integrator Option" && (
          <TextField // Integrator Options default to double only
            id="parameterType"
            key="parameterType"
            fullWidth
            label={`${label} Type`}
