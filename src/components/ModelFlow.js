...
export default function ModelFlow({
  ...
  const onLayout = useCallback(
    (direction) => {
      ...
      const { nodes: recenteredNodes, edges: recenteredEdges } = recenterAssets(
        layoutedNodes,
        layoutedEdges,
      );
      ...
      if (
        componentList.find(
          (component) =>
            ...
            (component.id === source || component.id === target),
        )
      ) {
        setErrorModalOpen(true);
  ...
  const onDrop = useCallback(
    (e) => {
      ...
      const { data, backgroundColor, newConstraints } = JSON.parse(
      ...
      if (parent === undefined) {
        ...
        setConstraints((prevConstraints) =>
          prevConstraints.concat(newConstraints),
        );
      }
        ...
        do {
          ...
          for (let i = 0; i < nodes.length; i++) {
            ...
            if (
              ...
              if (
                Math.abs(node.position.x - newNode.position.x) < 150 &&
                Math.abs(node.position.y - newNode.position.y) < 40
              ) {
                overlapped = true;
