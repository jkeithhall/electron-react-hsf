...
export default function ComponentEditor({
  ...
  const handleDeleteSubComponent = useCallback(() => {
    setComponentList((prevList) =>
      prevList.filter((component) => component.id !== deleteId),
    );
    setDependencyList((prevList) =>
      prevList.filter(
        (dependency) =>
          dependency.subsystem !== deleteId &&
          dependency.depSubsystem !== deleteId,
      ),
    );
    setConstraints((prevConstraints) =>
      prevConstraints.filter((constraint) => constraint.subsystem !== deleteId),
    );
    setEvaluator((prevEvaluator) => {
      const { keyRequests } = prevEvaluator;
      const newKeyRequests = keyRequests.filter(
        (keyRequest) => keyRequest.subsystem !== deleteId,
      );
    ...
    setEdges((prevEdges) =>
      prevEdges.filter(
        (edge) => edge.source !== deleteId && edge.target !== deleteId,
      ),
    );
  ...
  const handleDeleteAsset = useCallback(() => {
    setComponentList((prevList) =>
      prevList.filter(
        (component) =>
          component.id !== deleteId && component.parent !== deleteId,
      ),
    );
    setDependencyList((prevList) =>
      prevList.filter(
        (dependency) =>
          dependency.asset !== deleteId &&
          dependency.depAsset !== deleteId &&
          dependency.subsystem !== deleteId &&
          dependency.depSubsystem !== deleteId,
      ),
    );
    setConstraints((prevConstraints) =>
      prevConstraints.filter(({ subsystem }) => {
    ...
    setEvaluator((prevEvaluator) => {
      const { keyRequests } = prevEvaluator;
      const newKeyRequests = keyRequests.filter(
        (keyRequest) => keyRequest.asset !== deleteId,
      );
      return { ...prevEvaluator, keyRequests: newKeyRequests };
    });
    setNodes((prevNodes) => {
      return prevNodes.filter(
        (node) => node.id !== deleteId && node.parentNode !== deleteId,
      );
    });
    setEdges((prevEdges) =>
      prevEdges.filter(
    ...
    <>
        ...
        >
          ...
          {noErrors && (
            <Box sx={{ position: "absolute", right: 0 }}>
              <Tooltip
                title={`Copy ${className ? "Component" : "Asset"} to Clipboard`}
              >
                <IconButton
      ...
      {confirmationModalOpen && (
        <ConfirmationModal
          ...
          onConfirm={
            !data.className ? handleDeleteAsset : handleDeleteSubComponent
          }
          title={`Delete ${data.name}?`}
          message={`Are you sure you want to delete ${data.name}?`}
          submessage={
            !data.className ? " This will also delete all subcomponents." : ""
          }
          confirmText="Delete"
          cancelText="Cancel"
        />
